...
class MethodDiagnostic:
    ...
    def load_and_prepare_data(self):
        ...
        # Create all three price measures
        self.df['log_price_usd'] = np.log(self.df['fixed_broad_price_usd'] + 1)
        self.df['log_price_gni'] = np.log(self.df['fixed_broad_price_gni_pct'] + 1)
        self.df['log_price_ppp'] = np.log(self.df['fixed_broad_price_ppp'] + 1)
        
        # Default to PPP for main analysis
        self.df['log_price'] = self.df['log_price_ppp']
        ...
        # Mobile price instruments (all three types)
        if 'mobile_broad_price_usd' in self.df.columns:
            self.df['log_mobile_price_usd'] = np.log(self.df['mobile_broad_price_usd'] + 1)
        if 'mobile_broad_price_gni_pct' in self.df.columns:
            self.df['log_mobile_price_gni'] = np.log(self.df['mobile_broad_price_gni_pct'] + 1)
        if 'mobile_broad_price_ppp' in self.df.columns:
            self.df['log_mobile_price_ppp'] = np.log(self.df['mobile_broad_price_ppp'] + 1)
            self.df['log_mobile_price'] = self.df['log_mobile_price_ppp']  # Default to PPP
    ...
    def generate_recommendations(self):
        ...
        if 'iv_2sls' in self.method_scores and self.method_scores['iv_2sls']['suitable']:
            ...
            if iv_score.get('first_stage_f', 0) > 10:
                recommendations.append({
                    ...
                    'issues': '; '.join(iv_score['issues']) if iv_score['issues'] else 'None'
                })
        ...
        if 'twoway_fe' in self.method_scores:
            recommendations.append({
                ...
                'issues': '; '.join(self.method_scores['twoway_fe']['issues'])
            })
        ...
        if 'fixed_effects' in self.method_scores:
            recommendations.append({
                ...
                'use_case': 'BASELINE',
                'issues': '; '.join(self.method_scores['fixed_effects']['issues'])
            })
        ...
        if 'pooled_ols' in self.method_scores:
            recommendations.append({
                ...
                'issues': '; '.join(self.method_scores['pooled_ols']['issues'])
            })
        ...
        for i, rec in enumerate(recommendations, 1):
            if rec['use_case'] in ['PRIMARY ESTIMATION', 'ROBUSTNESS CHECK', 'BASELINE']:
                ...
                if rec['issues'] and rec['issues'] != 'None':
                    print(f"   ⚠️  Issues: {rec['issues']}")
