...
UPDATED: Variable names now consistent with 04_prepare_data.py output
================================================================================
"""
...
class DescriptiveAnalysis:
    ...
    def summary_statistics(self, df):
        ...
        # Key variables for analysis (UPDATED NAMES)
        key_vars = [col for col in [
            'fixed_broad_price',  # ← UPDATED from 'price_fixed_bb'
    ...
    def regional_comparison(self, df):
        ...
        # Variables to compare (UPDATED NAMES)
    ...
    def time_trends(self, df):
        ...
        # Variables to plot (UPDATED NAMES)
        plot_vars = {
            'fixed_broad_price': 'Fixed Broadband Price (% GNI)',  # ← UPDATED
    ...
    def scatter_plots(self, df):
        ...
        # Plot 2: Original scale (UPDATED NAMES)
        if 'fixed_broad_price' in df.columns and 'bandwidth_use' in df.columns:
            for region in ['EU', 'EaP']:
                data = df[df['region'] == region]
                ax2.scatter(data['fixed_broad_price'], data['bandwidth_use'],  # ← UPDATED
                           alpha=0.5, label=region, s=30)

            ax2.set_xlabel('Price (% GNI per capita)', fontsize=11)
...
def main():
    ...
    print("="*80)
    print("DESCRIPTIVE STATISTICS SCRIPT (UPDATED)")
    print("="*80)
