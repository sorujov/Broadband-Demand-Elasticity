# code/data_preparation/04_prepare_data.py (FIXED)
"""
================================================================================
Data Preparation Script (UPDATED & FIXED)
================================================================================
Purpose: Clean, impute, and prepare analysis-ready dataset
Author: Samir Orujov
Date: November 14, 2025 (Fixed)

UPDATED: Now compatible with series-metadata-preserved data structure
         Allows selection of specific ITU series for analysis
FIXED: Mobile price now properly included in series_config

This script:
1. Loads merged data with series metadata
2. Selects appropriate series for analysis (including mobile price!)
...
class DataPreparator:
    ...
    def __init__(self, series_config=None):
        ...
        series_config : dict, optional
            ...
            Example:
            {
                ...
                'mobile_broad_price': 'i271mb_ts_PPP'  # ← CORRECT series code
            }
        ...
        self.default_series = {
            'fixed_broad_price': 'i154_FBB_ts_PPP',
            'mobile_broad_price': 'i271mb_ts_PPP',  # FIXED: Correct series code
        }
    ...
    def select_and_rename_series(self, df):
        ...
        for var_base, series_code in self.series_config.items():
            ...
            if full_col_name in df.columns:
                ...
                print(f"    Non-null values: {df[var_base].notna().sum()}")
            else:
                print(f"  ⚠ {var_base}: Series {series_code} not found")
                print(f"    Looking for column: {full_col_name}")

                # Try to find similar columns
                similar = [col for col in df.columns if var_base in col and series_code.split('_')[0] in col]
                if similar:
                    print(f"    Similar columns found: {similar[:3]}")
                    print(f"    Hint: Check if series code is correct")
    ...
    def create_transformations(self, df):
        ...
        log_vars = {
            ...
            'mobile_broad_price': 'log_mobile_price',  # Add mobile price log
    ...
    def create_lags(self, df):
        ...
        lag_vars = ['bb_subs_per100', 'bandwidth_use', 'gdp_per_capita', 
                   'fixed_broad_price', 'mobile_broad_price']  # Add mobile
    ...
    def generate_summary_stats(self, df):
        ...
        key_vars = []
        for var in ['fixed_broad_price', 'mobile_broad_price', 'bandwidth_use', 
                    'bb_subs_per100', 'gdp_per_capita']:
            if var in df.columns:
                key_vars.append(var)
    ...
    def save_clean_data(self, df):
        ...
        print("\n" + "="*80)
        print("INSTRUMENT AVAILABILITY FOR IV ESTIMATION")
        print("="*80)

        instruments = {
            'regulatory_quality_estimate': 'Regulatory Quality',
            'mobile_broad_price': 'Mobile Broadband Price',
            'population_density': 'Population Density',
            'urban_population_pct': 'Urban Population %'
        }

        available_instruments = []
        for var, desc in instruments.items():
            if var in df.columns:
                non_null = df[var].notna().sum()
                pct = (non_null / len(df)) * 100
                print(f"  ✓ {desc}: {non_null}/{len(df)} ({pct:.1f}%)")
                available_instruments.append(var)
            else:
                print(f"  ✗ {desc}: NOT AVAILABLE")

        print(f"\nTotal instruments available: {len(available_instruments)}")

        if len(available_instruments) == 0:
            print("  ⚠⚠ NO INSTRUMENTS - IV estimation not possible")
        elif len(available_instruments) == 1:
            print("  ⚠ Only 1 instrument - Cannot test overidentification")
        else:
            print(f"  ✓ {len(available_instruments)} instruments - Can test overidentification")

        # Save descriptive stats
        stats_file = DATA_PROCESSED / 'descriptive_stats_by_region.csv'
        key_vars = [col for col in ['fixed_broad_price', 'mobile_broad_price', 
                                     'bandwidth_use', 'bb_subs_per100',
...
def main():
    ...
    print("="*80)
    print("DATA PREPARATION SCRIPT (UPDATED & FIXED)")
    print("="*80)
    print(f"Execution time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # Series configuration for main analysis
    # IMPORTANT: Document these choices in your paper!
    # FIXED: Mobile price now included with correct series code
    series_config = {
        'fixed_broad_price': 'i154_FBB_ts_PPP',      # PPP-adjusted (recommended)
        'mobile_broad_price': 'i271mb_ts_PPP'        # Mobile price (IV instrument) ← ADDED!

        # Alternative series for robustness checks:
        # 'fixed_broad_price': 'i154_FBB_ts_GNI',  # GNI-based
        # 'fixed_broad_price': 'i154_FBB_ts$',     # USD-based
    }
    ...
    for var, series in series_config.items():
        print(f"  {var}: {series}")
    print("\n✓ Mobile broadband price INCLUDED for IV estimation")
    ...
    try:
        ...
        print("  → IV/2SLS estimation (with mobile price instrument!)")
